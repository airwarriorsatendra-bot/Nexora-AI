...
def render_ai_visibility(workflow=None):
 ...
 if not isinstance(report,VisibilityReport):st.info("Add or select prompts, review the API-call preview, then run monitoring explicitly. No provider call occurs on rerender.");return
 ...
 history=run(workflow.history());history_frame=pd.DataFrame([{"Prompt":o.prompt,"Provider":o.provider,"Model":o.model,"Brand mentioned":o.brand_mention is not None,"Target cited":o.target_domain_cited,"State":o.state.value,"Observed at":o.observed_at} for o in history]);st.subheader("History");st.dataframe(history_frame,hide_index=True,width="stretch")
