...
class LocalSEOSettings:
 ...
 @property
 def gbp_configured(self):return all((self.gbp_client_id,self.gbp_client_secret,self.gbp_refresh_token))
...
class LocalSEOComposition:
 def __init__(self,settings:LocalSEOSettings,*,crawler_factory=CrawlerService,repository_factory=LocalSEORepository,business_profile_factory=OfflineBusinessProfileProvider,review_factory=OfflineReviewProvider,rank_factory=OfflineLocalRankProvider,citation_factory=OfflineCitationProvider,competitor_factory=OfflineLocalCompetitorProvider):self.s=settings;self.crawler_factory=crawler_factory;self.repository_factory=repository_factory;self.factories=(business_profile_factory,review_factory,rank_factory,citation_factory,competitor_factory)
 def build(self):
  repo=self.repository_factory(self.s.database_path);crawler=self.crawler_factory();service=LocalSEOAuditService(crawler.fetch_html,repo);providers=[factory() for factory in self.factories];return LocalSEOApplication(service,repo,*providers)
