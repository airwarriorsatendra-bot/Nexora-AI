...
class ContentIntelligenceService:
 ...
 def generate(self,gap:KeywordGap,all_gaps=(),page=None,links=()):
  ...
  question_type=AEOGEOService.question_type(gap.keyword);question_opportunities=(f"Persisted {question_type.value.replace('_',' ').lower()} question: {gap.keyword}",) if question_type else ();aeo=[];direct=[];faq=[];geo=[];entity=[]
  if question_type:
   aeo.append("Review this mapped question as an answer-readiness opportunity using the persisted GSC and tracked-SERP evidence.");direct.append("Consider a concise, evidence-backed answer near a relevant heading.")
   if question_type.value=="HOW":direct.append("Consider structured steps if the page content genuinely describes a procedure.")
   faq.append("Consider FAQ content only for distinct user questions; FAQ rich-result eligibility is not guaranteed.")
  if page:
   geo.append("Review extractability using the observed title, H1, metadata, schema, and technical accessibility signals.")
   if page.external_links:entity.append(f"{page.external_links} outbound links were observed; source quality was not evaluated.")
   else:entity.append("No outbound source link was observed; add credible primary references where factual claims require support.")
   if not page.structured_data_types:entity.append("No structured data type was observed; review only schema supported by visible content.")
  return ContentBrief(target_url=gap.mapped_page,mode=mode,primary_query=gap.keyword,primary_query_reason="Explicit selected tracked keyword with persisted competitor-gap evidence.",priority=priority,score=ContentScore(search=search,serp=serp,page_readiness=readiness,engagement=engagement,gap=gap_score,total=total),gsc_impressions=gap.gsc_impressions,gsc_clicks=gap.gsc_clicks,gsc_ctr=gap.gsc_ctr,gsc_average_position=gap.gsc_average_position,tracked_position=gap.target_position,competitors_ahead=gap.competitors_ahead,intent=intent,intent_evidence=intent_evidence,current_title=page.title if page else None,current_meta=page.meta_description if page else None,current_h1=page.h1s[0] if page and page.h1s else None,crawl_depth=page.depth if page else None,inlinks=page.inlink_count if page else None,technical_issues=issues,supporting_queries=tuple(support),serp_competitors=tuple(SERPCompetitor(domain=r.domain,position=r.position,url=r.url,title=r.title,snippet=r.snippet) for r in gap.serp if not r.is_target),content_gap_observations=tuple(observations),title_guidance=title_guidance,suggested_title=suggested,meta_guidance=meta_guidance,suggested_h1=gap.keyword.title(),h2_sections=tuple(h2),internal_links=tuple(recs),technical_preconditions=tuple(pre),actions=tuple(f"{i+1}. {a}" for i,a in enumerate(actions)),aeo_opportunities=tuple(aeo),geo_readiness=tuple(geo),question_opportunities=question_opportunities,direct_answer_suggestions=tuple(direct),faq_opportunities=tuple(faq),entity_source_support=tuple(entity),evidence=tuple(BriefEvidence(source="COMPETITOR_GAP",observation=e) for e in gap.evidence),limitations=("Search volume, keyword difficulty, competitor traffic, authority metrics, and guaranteed outcomes are not available.","Intent is a cautious observation from persisted query and SERP patterns.","AEO/GEO recommendations are Nexora readiness heuristics, not ranking or AI-citation guarantees."))
 @staticmethod
 def markdown(b):
  def lines(values):return "\n".join(f"- {v}" for v in values) or "- NOT AVAILABLE"
  return f"# SEO Content Brief: {b.primary_query}\n\n## Overview\n- Mode: {b.mode.value}\n- Target URL: {b.target_url or 'NOT AVAILABLE'}\n- Priority: {b.priority.value}\n- Score: {b.score.total}/100\n\n## Intent observation\n- {b.intent.value}\n{lines(b.intent_evidence)}\n\n## Title guidance\n{lines(b.title_guidance)}\n- Suggested copy: {b.suggested_title}\n\n## Meta guidance\n{lines(b.meta_guidance)}\n\n## Heading outline\n- H1: {b.suggested_h1}\n{lines(b.h2_sections)}\n\n## AEO opportunities\n{lines(b.aeo_opportunities)}\n\n## GEO readiness\n{lines(b.geo_readiness)}\n\n## Question opportunities\n{lines(b.question_opportunities)}\n\n## Direct answer suggestions\n{lines(b.direct_answer_suggestions)}\n\n## FAQ opportunities\n{lines(b.faq_opportunities)}\n\n## Entity / source support\n{lines(b.entity_source_support)}\n\n## Technical preconditions\n{lines(b.technical_preconditions)}\n\n## Action plan\n{lines(b.actions)}\n\n## Limitations\n{lines(b.limitations)}\n"
